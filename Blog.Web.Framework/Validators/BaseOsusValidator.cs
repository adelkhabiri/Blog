...
namespace Blog.Web.Framework.Validators
{
    public abstract class BaseOsusValidator<T> : AbstractValidator<T> where T : class
    {
        protected BaseOsusValidator()
        {
            PostInitialize();
        }
        ...
        protected virtual void SetDecimalMaxValue<TObject>(IDbContext dbContext)
        {
            ...
            foreach (var expr in expression)
            {
                RuleFor(expr.Value).IsDecimal(maxValues[expr.Key]).WithMessage(string.Format(localizationService.GetResource("Blog.Web.Framework.Validators.MaxDecimal"), maxValues[expr.Key] - 1));
            }
        }
    }
}