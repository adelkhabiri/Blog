            ...
            for (int i = 0; i < attributes.Count; i++)
            {
                ...
                for (int j = 0; j < valuesStr.Count; j++)
                {
                    ...
                    if (!attribute.ShouldHaveValues())
                    {
                        ...
                        {
                            //multiline textbox
                            var attributeName = attribute.GetLocalized(a => a.Name, 1/*_workContext.WorkingLanguage.Id*/);
                            //encode (if required)
                            if (htmlEncode)
                        ...
                        else
                        {
                            ...
                            formattedAttribute = string.Format("{0}: {1}", attribute.GetLocalized(a => a.Name, 1/*_workContext.WorkingLanguage.Id*/), valueStr);
                            //encode (if required)
                            if (htmlEncode)
                    ...
                    else
                    {
                        ...
                        if (int.TryParse(valueStr, out attributeValueId))
                        {
                            ...
                            {
                                formattedAttribute = string.Format("{0}: {1}", attribute.GetLocalized(a => a.Name,1 /*_workContext.WorkingLanguage.Id*/), attributeValue.GetLocalized(a => a.Name,1/* _workContext.WorkingLanguage.Id*/));
                            }
                            //encode (if required)
                            if (htmlEncode)
                                formattedAttribute = HttpUtility.HtmlEncode(formattedAttribute);
