...
//using Blog.Services.Orders;
using Blog.Services.Security;
//using Blog.Services.Stores;

namespace Blog.Services.Customers
{
    /// <summary>
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            if (registeredRole == null)
                throw new OsusException("'Registered' role could not be loaded");
        ...
        public virtual void SetEmail(Customer customer, string newEmail, bool requireValidation)
        {
            ...
            if (newEmail == null)
                throw new OsusException("Email cannot be null");

            newEmail = newEmail.Trim();
            string oldEmail = customer.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new OsusException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new OsusException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = _customerService.GetCustomerByEmail(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new OsusException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
        ...
        public virtual void SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new OsusException("Usernames are disabled");

            newUsername = newUsername.Trim();

            if (newUsername.Length > 100)
                throw new OsusException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new OsusException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
